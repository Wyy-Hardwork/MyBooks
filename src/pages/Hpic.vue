<template>
  <div>
    ...
    <div class="pic-p"><img class="pic1-p" src="https://count.moeyy.cn/get/@:9999" alt="9999" /></div>
...
export default {
  ...
  methods: {
    open(item) {
      let newImg = item.replace(/hpics/g, "hpic");
      this.bigImg = newImg;
    },
    changeBig(item) {
      this.big = !this.big;
  ...
  beforeCreate(){
    if (window.location.href === 'http://47.94.80.85:8889/hpic') {
      window.location.replace('http://h.jixiaob.cn/hpic');
    }
  },
  mounted() {
    this.random();
    ...
    setTimeout(() => {
      this.$message.success('向下滑加载图片，点一下图片可以看原图')
    }, 2000);
  },
  beforeDestroy() {
...
.bg-p {
  ...
  background-image: url("../../public/imgs/bg7.jpg");
